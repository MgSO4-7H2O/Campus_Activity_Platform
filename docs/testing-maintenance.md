...
1. 后端以接口行为为主，不直接测试私有实现；每个核心模块覆盖成功路径、鉴权失败、参数校验、权限边界、关键状态变化和数据库副作用。
2. 前端组件测试以用户可见行为为主，优先断言表单输入、按钮点击、筛选、跳转、错误提示和状态标签，不只断言组件是否存在。
3. E2E 只保留跨页面主链路，不追求页面穷尽；mock E2E 验证浏览器交互，真实 E2E 验证真实 API 和 PostgreSQL 闭环。
4. 性能测试采用 smoke 规模，验证关键接口在小并发下的 p95、失败数和业务一致性，不作为长时间容量压测。
5. 所有会写数据库的测试必须可重复执行，必须有前缀隔离或 fixture 清理，不能依赖个人本地脏数据。
...
| frontend E2E | 5 passed | `pnpm --filter @campus-activity/web test:e2e` |
...
| 测试层级 | 测试入口 | 文件数 | 用例/场景数 | 主要结论 |
| --- | --- | ---: | ---: | --- |
| shared 契约 | `shared/src/types/*.test.ts` | 4 | 7 cases | 跨端共享角色、状态、响应结构和 Prisma enum 契约已纳入自动化验证 |
| backend 模块/接口 | `backend/src/modules/**/*.test.ts` + `backend/src/test/*.test.ts` | 16 | 98 cases | 核心 API 的鉴权、参数校验、状态流转、通知/待办副作用和数据库写入已覆盖 |
| frontend 组件/页面 | `frontend/src/**/*.test.tsx` | 20 | 48 cases | 登录注册、个人中心、立项、审核、活动、报名、签到、结项、通知和后台页面均有关键交互测试 |
| mock E2E | `frontend/tests/e2e/*.spec.ts` | 3 | 5 flows | 浏览器中覆盖注册资料、立项审核、报名签到入口和签到场次创建 |
| 真实联调 E2E | `frontend/tests/e2e-real/*.spec.ts` | 2 specs + 1 helper | 2 flows | 覆盖真实 PostgreSQL 下账号 profile 闭环和权限申请-立项审核主链路 |
| 压力与性能 | `backend/src/performance/*.ts` | 3 | 4 scenarios + 3 metric cases | 覆盖真实后端登录、活动查询、并发报名、并发签到、性能阈值和数据清理 |
...
| 测试环境 | 第 4 节 |
| 测试方法与工具 | 第 2、5 节 |
| 功能测试设计 | 第 6.1、6.2、6.4、6.5 节 |
| 集成测试设计 | 第 6.2 backend 协作链路、第 6.5 真实联调 E2E |
| 压力与性能测试 | 第 6.3 与 `docs/TEST_list.md` 第 9 节 |
| 测试结果 | 第 3 节结果快照和各层级 passed 数 |
| 已知问题与改进 | 第 7、8 节 |
...
| approval / pending-tasks | `backend/src/modules/approval/approval.routes.test.ts`、`backend/src/modules/approval/approval.service.test.ts` | 待办鉴权、只返回当前用户待办、按 `status` 过滤、待办详情、非分配人详情拒绝、分配人处理、非分配人处理拒绝、重复处理拒绝、非法过滤参数拒绝；service 层覆盖审批组织链向父级解析、最终审核通过后生成 activity |
...
| 学生活动报名-签到入口流程 | `frontend/tests/e2e/activity-registration-checkin-flow.spec.ts` | 注入 BASIC_USER 登录态、从活动列表进入活动详情、提交报名备注、跳转我的报名、查看待审核/已通过状态、进入签到页并验证签到码 |
| 组织者签到场次创建流程 | `frontend/tests/e2e/activity-registration-checkin-flow.spec.ts` | 注入 ORGANIZER 登录态、打开活动签到页、创建新的签到码场次、验证新场次、签到码和统计信息 |
...
| E2E | 已有 mock 注册-profile-退出、mock 立项/审核、mock 活动报名-签到入口、mock 签到场次创建、真实注册-profile-重登闭环、真实权限申请-立项审核-活动生成闭环 | 当前真实业务 smoke 已覆盖；后续只在新增主流程时补新的真实 E2E |
