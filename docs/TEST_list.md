...
| mock E2E | 5 passed | `pnpm --filter @campus-activity/web test:e2e` |
...
最近一次验证耗时记录：shared 0.56s、backend 16.33s、frontend 13.02s、mock E2E 4.1s、real E2E 7.1s、性能烟测 1.43s。

测试报告可引用的当前测试资产规模：

| 测试层级 | 测试文件数 | 用例/场景数 | 报告口径 |
| --- | ---: | ---: | --- |
| shared 契约测试 | 4 | 7 cases | 白盒/契约测试，验证跨端类型、状态常量、响应结构和 Prisma enum 同步 |
| backend 模块与接口测试 | 16 | 98 cases | 接口测试、业务规则测试、数据一致性测试，覆盖鉴权、参数校验、状态流转和数据库写入 |
| frontend 组件与页面测试 | 20 | 48 cases | 前端单元/组件测试，覆盖页面渲染、表单、筛选、路由和用户反馈 |
| mock E2E | 3 | 5 flows | 浏览器端黑盒流程测试，覆盖注册资料、立项审核、报名签到和签到场次创建 |
| 真实后端 + PostgreSQL E2E | 2 specs + 1 helper | 2 flows | 端到端联调测试，验证浏览器、真实 API 和真实数据库闭环 |
| 压力与性能烟测 | 3 | 4 scenarios + 3 metric cases | 性能与稳定性专项测试，验证真实后端关键接口 p95、失败数、业务一致性和清理能力 |

测试报告建议按以下口径归类：

- 功能测试：第 3、4、6、7 节中的 shared、backend、frontend、E2E 测试点。
- 集成测试：第 5 节 backend 协作链路、第 7 节真实后端 + PostgreSQL E2E。
- 性能测试：第 9 节压力与性能烟测。
- 测试环境与数据隔离：第 8 节。
- 问题与风险：第 10 节。
...
1. `pnpm typecheck`
2. `pnpm --filter @campus-activity/shared test:run`
3. `pnpm --filter @campus-activity/server test:run`
4. `pnpm --filter @campus-activity/web test:run`
5. `pnpm --filter @campus-activity/web test:e2e`
6. `pnpm --filter @campus-activity/web test:e2e:real`
7. `pnpm --filter @campus-activity/server test:perf`
8. `pnpm lint`
...
| [x] | approval | `backend/src/modules/approval/approval.routes.test.ts`、`backend/src/modules/approval/approval.service.test.ts` | 未登录查询待办失败、只返回当前分配待办、按 status 过滤、分配人查询详情、非分配人查询失败、分配人处理待办、非分配人处理失败、已处理待办重复处理失败、不支持过滤参数返回校验错误；service 层验证审批组织链向父级解析、最终审核通过后生成 activity |
...
| [x] | mock 学生活动报名-签到入口 | `frontend/tests/e2e/activity-registration-checkin-flow.spec.ts` | 注入 BASIC_USER 登录态、从活动列表进入活动详情、进入报名页提交备注、跳转我的报名、查看报名状态、进入活动签到页并验证签到码 |
| [x] | mock 组织者签到场次创建 | `frontend/tests/e2e/activity-registration-checkin-flow.spec.ts` | 注入 ORGANIZER 登录态、打开活动签到页、创建签到码场次、验证新场次 tab、签到码和统计信息 |
...
| [x] | 当前不追求穷尽文件上传、分页、移动端布局和长时间持续压测 | 已保留关键自动化回归；只在对应功能变更或验收要求扩大时补专项用例 |
